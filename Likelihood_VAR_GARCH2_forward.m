function [Loglf] = Likelihood_VAR_GARCH2_forward(X,para,p,h,tt)
...
lf = zeros(tt-1,1);  % store the probability density of x_t

H = zeros(tt-1,m);
V = zeros(tt-1,m);

if abs(1/(tt-1)-1) <= h
    H(1,:) = (c0+c0d*(1/(tt-1)-1)/h)';
    V(1,:) = X(1,:);
...
for t = 2:tt-1    
    ...
    if t >= p+1
        tau_t = t/(tt-1);
        if abs(tau_t-1) <= h
            V(t,:) = X(t,:) - X(t-1,:)*(A1+A1d*(tau_t-1)/h)';
            if p==1
                aa = (V(t-1,:).*V(t-1,:))';
            elseif p>1
                %aa = mpower(D1+D1d*(tau_t-1)/h,p-1)*(C1+C1d*(tau_t-1)/h)*(V(t-p,:).*V(t-p,:))'; % 若想改候选模型，只需要改这里即可
            ...
            %end
            H(t,:) = (c0+c0d*(tau_t-1)/h)'* pinv(eye(m)-D1)+ aa';
            % 上面  这里Vt-1 滞后阶数为相应候选模型的滞后阶数
            lf(t,1) = V(t,:)*pinv(mpower(diag(H(t,:)),0.5)*(Omega+Omegad*(tau_t-1)/h)*mpower(diag(H(t,:)),0.5))*V(t,:)' + log(det(mpower(diag(H(t,:)),0.5)*(Omega+Omegad*(tau_t-1)/h)*mpower(diag(H(t,:)),0.5)));
        end
    end
end

K = K_weight(tt-1,1,h);   % kernel weighting matrix
Loglf = sum(K*lf)/(tt-1);
end
