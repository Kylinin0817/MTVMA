function [RMSE] = simulationfcn(T,h,Pmax)
rng(0);
m = 2;
d = m*(m+1)/2;
K = 1000; %重复试验次数
...
parfor k = 1:K %并行
    ...
    for p = 1:Pmax  % 这里等运行完改回来 1：Pmax
        ...
        Sigma_p = zeros(m,m,T-p);  % 用于记录T-p个样本下的协方差矩阵的vech所以是每行都是1*d
        H_p = zeros(T-p,2);
        V_p = zeros(T-p,2);
        % Omega_p = zeros((T-p),d);
        ...
        for t = 1:T-p
            ...
            [para1,~,~,~,~,~] = fminunc(@(para)Likelihood_VAR_RGARCH2(X,para,p,tau,h_opt),para0,optimset('Display','off','MaxIter', 1000));
            ...
            if t==1
                ...
                V_p(t,:) = X_p(t,:);
            elseif t>1
                V_p(t,:) = X_p(t,:) - X_p(t-1,:)*A1_t_p';
                H_p(t,:) = c0_t_p' + (V_p(t-1,:).*V_p(t-1,:))*C1_t_p' + H_p(t-1,:)*D1_t_p';
            end
            Sigma_p(:,:,t) = mpower(diag(H_p(t,:)),0.5)*Omega_t_p*mpower(diag(H_p(t,:)),0.5);
            Y_fit_p(t,:) = X_lag(t,:)*A1_t_p'; % mu的话，只用到了A1
            eta_hat_p(t,:) = X_p(t,:) - Y_fit_p(t,:);  
            % 也就是只有这T-p个时间点都有残差值
        end
       ...
       % end
        ...
        Y_pred(:,p) = Y_fit_p(T-p,:)';  % 最后一个时刻的估计值，在时变多元因果过程的模型下
        tau_t_omega = 1;  % 因为是将最后一个时刻作为了测试样本，所以tau=1
        K_Ome = K_weight(T-p, tau_t_omega, h_opt);   % 其他时刻的样本对估计测试样本起到的作用矩阵；算第p个候选模型下的协方差的估计时用的样本点是最新的样本
    ...
    QQ = zeros(Pmax, Pmax);
    K_ma = K_weight((T-Pmax), 1, h_opt_ma); % 这里为什么tau取1呢
    %这里权重选取准则得改！！!!!!!!!
    for j = 1:(T-Pmax)  % eta_hat: T-Pmax,m,Pmax; Omega_hat: m,m,Pmax; K_ma: T-Pmax, T-Pmax
    ...
    for p = 1:Pmax
        ...
        AIC(1, p) = log(det(squeeze(Omega_residual))) + 2 * p * m^2 / (T-Pmax);
        BIC(1, p) = log(det(squeeze(Omega_residual))) + log(T-Pmax) * p * m^2 / (T-Pmax);
        HQ(1, p) = log(det(squeeze(Omega_residual))) + 2 * log(log(T-Pmax)) * p * m^2 / (T-Pmax);
    end
